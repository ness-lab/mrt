...
class SFS:
    ...
    def allele_count(self):
        if self.alleles:
            return self.alleles
        if self.folded:
            self.alleles = 2 * (len(self.sfs) - 1)
        else:
            self.alleles = len(self.sfs) - 1
        return self.alleles

    def invariant(self):
        return self.sfs[0] + self.sfs[-1]

    def sites(self):
        return sum(self.sfs)

    def variant(self):
        return self.sites() - self.invariant()
    ...
    def fold(self):
        for i in range(int(len(self.sfs) / 2)):
            self.sfs[i], self.sfs[-i - 1] = self.sfs[i] + self.sfs[-i - 1], 0
        self.folded = True
        return self
        # i = 1
    ...
    def summarise(self, header=True):
        ...
        sys.stdout.write("\t".join([str(i) for i in [self.alleles, self.folded, self.sites(), self.invariant(), self.variant(), self.theta_pi(), self.theta_w(), self.taj_D(), self.sfs]]) + "\n")
